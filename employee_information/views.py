...
def reports(request):
    ...
    today_registerd_employees = Employees.objects.filter(
        today_registed=day_month_year).values().count()
    active_employees = Employees.objects.filter(status=1).values().count()
    deactive_employees = Employees.objects.filter(status=0).values().count()
    no_male_employees = Employees.objects.filter(
        gender='Male').values().count()
    no_female_employees = Employees.objects.filter(
        gender='Female').values().count()
    context = {
        'page_title': 'Reports',
        'total_employee': len(Employees.objects.all()),
        'active_employees': active_employees,
        'deactive_employees': deactive_employees,
        # 'today_registerd_employees': today_registerd_employees,
        'day_month_year': day_month_year,
        'no_male_employees': no_male_employees,
        'no_female_employees': no_female_employees

    }
    return render(request, 'employee_information/reports.html', context)
...
def messageApi(request):
    ...
    return render(request, 'employee_information/message.html')
